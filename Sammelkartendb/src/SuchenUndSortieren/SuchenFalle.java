...
public class SuchenFalle {
	...
	static Card[] BinarySearchFalle(Trap[] arr, int start, int stop, SortType at, String wertString) throws Exception {
		if(arr.length == 0)
	...
	static Card[] FibonacciSearchFalle(Trap[] arr, int i, int length, SortType at, String wertString) throws Exception {
		if(arr.length == 0)
			throw new Exception("Keine Karten in der Datenbank"); //Falls die Datenbank leer ist
		List<Card> cardList = new Vector<Card>(0,1);
		int offset = -1;
	...
	static Card[] ExponentialSearchFalle(Trap[] arr, int i, int length, SortType at, String wertString) throws Exception {
		if(arr.length == 0)
			throw new Exception("Keine Karten in der Datenbank"); //Falls die Datenbank leer ist
		if(at.name().equals("TYPE")) {
			...
			return BinarySearchFalle(arr, ex/2, Math.min(ex, arr.length-1), at, wertString);
		} else {
			throw new Exception("Gewähltes Attribut passt nicht zum Kartentyp Falle"); //Sollte im fertigen Programm nicht eintreten können
		}
	}

	static Card[] InterpolationSearchFalle(Trap[] arr, int start, int stop, SortType at, String wertString) throws Exception {
		if(arr.length == 0)
			throw new Exception("Keine Karten in der Datenbank"); //Falls die Datenbank leer ist
		List<Card> cardList = new Vector<Card>(0,1);
		int pos;
		if(at.name().equals("TYPE")) { //Name
			if (start <= stop && wertString.compareToIgnoreCase(arr[start].getType()) >= 0 && wertString.compareToIgnoreCase(arr[stop].getType()) <= 0) {
				...
				if (arr[pos].getType() == wertString) { //gesuchter Wert gefunden
					...
					return InterpolationSearchFalle(arr, pos+1, stop, at, wertString);
					...
					return InterpolationSearchFalle(arr, start, pos-1, at, wertString);
				}
			}
			return (Card[]) cardList.toArray(); //gesuchter Wert nicht gefunden. Leeres Array wird zurückgegeben
		} else {
